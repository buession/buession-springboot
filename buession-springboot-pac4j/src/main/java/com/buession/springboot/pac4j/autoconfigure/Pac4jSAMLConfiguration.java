...
public class Pac4jSAMLConfiguration extends AbstractPac4jClientConfiguration<Saml> {
	...
	public SAML2Client saml2Client(ObjectProvider<Customizer<SAML2Client>> customizers) {
		...
		nonNullpropertyMapper.from(config::getAjaxRequestResolver).as(BeanUtils::instantiateClass)
				.to(saml2Client::setAjaxRequestResolver);
		customizers.orderedStream().forEach((customizer)->customizer.customize(saml2Client));
